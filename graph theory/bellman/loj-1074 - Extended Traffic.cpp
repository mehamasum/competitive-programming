...
void bellman()
{
	...
	for(int i=1;i<=v-1;i++)
	{
		...
		for(int j=1;j<=e;j++)
		{
			...
			if(d[a]==INF && edge_cost[j]<0) continue;
...
int main()
{
	...
	FRAE(t,1,T)
	{
		...
		FRAE(i,1,e) 
		{
			sff(edge_u[i],edge_v[i]);
			edge_cost[i]=(w[edge_v[i]]-w[edge_u[i]])*(w[edge_v[i]]-w[edge_u[i]])*(w[edge_v[i]]-w[edge_u[i]]);
			//dfff(edge_u[i],edge_v[i],edge_cost[i]);
		}
