...
void kmp(const string &key, const string &text) 
{
	int m = key.size();
	...
	{
		table[i+1] = table[i];
		while (table[i+1] > -1 && key[table[i+1]] != key[i]) 
			table[i+1] = table[table[i+1]];
		table[i+1]++;
	}

	//printArr(table,0,m+1);

	int n = text.size();
	int seen = 0;
	for (int i = 0; i < n; ++i)
	{
		while (seen > -1 && key[seen] != text[i]) 
			seen = table[seen];
		if (++seen == m) 
		{
			printf("%d\n", i - m + 1);
