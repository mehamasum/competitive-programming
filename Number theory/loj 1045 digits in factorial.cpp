...
int main()
{
	...
	LOG[1]=0;
	FRA(i,2,MAX) LOG[i]=log10(i*1.0)+LOG[i-1];
	...
	FRAE(t,1,T)
	{
		...
		sff(n,b);
		if(n==0) pf("Case %d: 1\n",t);
		else pf("Case %d: %d\n",t,(int)floor(LOG[n]/log10(b*1.0))+1);
		
	}
	return 0;
}

//        fact_n=n!=n*n-1*n-2*..........*3*2*1;
//        log(fact_n)=log(n*n-1*n-2*..........*3*2*1);
// LOG[n]=log(fact_n)=log(n)+log(n-1)+.....+log(1);
//        log(fact_n+1)=log(n+1)+log(fact_n);
// floor[log(fact_n)/log(b)]+1